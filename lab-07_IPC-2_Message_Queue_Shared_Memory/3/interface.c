...
int main() {
    ...
    printf("Interface program started. Type 'exit' to quit.\n");

    while(running) {
        while(shared_stuff->message_processed == 0) sleep(1);
        ...
        if(strncmp(buffer, "exit", 4) == 0) {
            printf("Exiting...\n");
            running = 0;
            strncpy(shared_stuff->message_text, buffer, TEXT_SZ);
            shared_stuff->message_ready = 1;
            shared_stuff->message_processed = 0;
            break;
        }
